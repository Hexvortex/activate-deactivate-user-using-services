...
from config.custom_components.user_access_control import SwitchEntity, PLATFORM_SCHEMA
import homeassistant.helpers.config_validation as cv

DOMAIN = "lockuser"

DEVICE_SCHEMA = vol.Schema(
    {vol.Required(CONF_NAME): cv.string, vol.Required(CONF_UNIQUE_ID): cv.string}
)
...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    for device_id, device_config in config.get(CONF_DEVICES, {}).items():
        ...
        _LOGGER.debug(f"Added switch '{name}', device ID: {device_id}, user ID: {uid}")
...
class CustSwitch(SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs):
        ...
        _LOGGER.debug(f"Switch '{self._name}' turned ON, user ID: {self._uid}")
    ...
    async def async_turn_off(self, **kwargs):
        ...
        self.async_schedule_update_ha_state()
        _LOGGER.debug(f"Switch '{self._name}' turned OFF, user ID: {self._uid}")
